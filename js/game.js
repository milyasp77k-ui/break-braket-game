...
(() => {
  ...
  let running = false, soundOn = true, musicOn = true;
  let paused = false;
  let levelIntroTime = 0;

  // Paddle
  const paddle = { baseW: 120, w: 120, h: 14, x: (W-120)/2, y: H - 60, speed: 10, expand: 1, sticky: false, stickyTimer: 0 };

  // Ball(s) - support simplified multiball as just one ball with speed change, but we keep array for clarity
  let balls = [{ r: 9, x: W/2, y: paddle.y - 12, vx: 0, vy: 0, stuck: true, speed: 6 }];
  ...
  function fitCanvas() {
    const containerW = Math.min(window.innerWidth - 64, 980);
  ...
  const levelDefs = [
    ...
    (L) => {
      ...
      for (let r=0;r<rows;r++){
        for (let c=0;c<cols;c++){
          ...
          if ((c<2||c>cols-3) || r===0) {
            if (Math.random()<0.09) arr.push({r,c,type:1}); else arr.push({r,c,type:0});
          } else {
    ...
    (L) => {
      ...
      for(let r=0;r<rows;r++){
        for(let c=0;c<cols;c++){
          if ((r+c)%2===0 && Math.random()<0.85) arr.push({r,c,type: (Math.random()<0.12?1:0)});
          else if (Math.random()<0.07) arr.push({r,c,type:2});
        }
      }
      return { rows, cols, arr };
    },
  ...
  function buildBricksForLevel(levelIndex) {
    ...
    balls = [{ r: 9, x: W/2, y: paddle.y - 12, vx:0, vy:0, stuck:true, speed: 6 + (level - 1) * 0.45 }];
  }
  ...
  function applyPowerup(type) {
    switch(type) {
      ...
      case 'multiball':
        ...
        if (balls.length < 3) {
          const main = balls[0];
          const b2 = { r: main.r, x: main.x+8, y: main.y-6, vx: -main.vx * 0.9, vy: main.vy * 0.9, stuck:false, speed: main.speed };
          balls.push(b2);
          score += 30;
        }
        break;
  ...
  function spawnPowerup(x,y) {
    const types = ['expand','multiball','extralife','slow','sticky','laser'];
    // weighted choose
    const r = Math.random();
    let type = 'expand';
    if (r<0.08) type = 'extralife';
    else if (r<0.20) type = 'multiball';
    else if (r<0.34) type = 'slow';
    else if (r<0.50) type = 'sticky';
    else if (r<0.58) type = 'laser';
    else type = 'expand';
  ...
  function update(t) {
    ...
    if (balls.every(b => b.stuck)) {
      for (const b of balls) { b.x = paddle.x + paddle.w / 2; b.y = paddle.y - 12; }
    }
    ...
    for (let bi = balls.length - 1; bi >= 0; bi--) {
      ...
      // wall collisions
      if (b.x - b.r < 6) { b.x = b.r + 6; b.vx *= -1; sfx(600, 50); }
      if (b.x + b.r > W - 6) { b.x = W - b.r - 6; b.vx *= -1; sfx(600, 50); }
      if (b.y - b.r < 6) { b.y = b.r + 6; b.vy *= -1; sfx(700, 60); }
  ...
  function render() {
    ...
    g.addColorStop(0, '#061024'); g.addColorStop(1, '#02101a');
    ctx.fillStyle = g; ctx.fillRect(0,0,W,H);

    // Bricks
    for (const b of bricks) {
      if (!b.alive) continue;
      if (b.type === 1) ctx.fillStyle = '#a78bfa';
      else if (b.type === 2) ctx.fillStyle = '#fb923c';
      else ctx.fillStyle = '#06b6d4';
      roundRect(ctx, b.x, b.y, b.w, b.h, 6, true, false);
      ctx.fillStyle = 'rgba(0,0,0,0.06)';
      ctx.font = '14px Inter, Arial';
      ctx.fillText('[ ]', b.x + 8, b.y + b.h - 8);
    }
    ...
    for (const p of powerups) {
      ...
      ctx.font = '12px Arial';
      ctx.fillText(p.type === 'expand' ? '+P' : (p.type==='extralife'?'+L': p.type==='multiball'? '*': (p.type==='slow'?'S':(p.type==='sticky'?'S*':'L'))), p.x-6, p.y+4);
    }
    ...
    ctx.fillStyle = '#a8f0e0'; ctx.font = 'bold 14px Inter, Arial'; ctx.fillText('Break Braket v2', 14, 28);
